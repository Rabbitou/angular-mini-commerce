...
@Component({
  ...
  template: `
    <section
      class="flex flex-col h-full mx-auto max-w-4xl justify-center items-center mt-2"
    >
      <div class="flex w-full justify-between p-2">
        <div
          class="bg-gray-200 h-9 flex items-center gap-1 text-gray-700 rounded-sm p-2"
        >
          ...
          <input
            ...
            name=""
            id=""
            class="outline-none bg-transparent max-sm:w-24"
            placeholder="Search..."
            (keyup)="search($event)"
          />
        </div>
        <select
          name="category"
          id="category"
          class="p-2 bg-gray-200 rounded-sm text-gray-700 outline-none capitalize max-sm:w-24"
      ...
      <div
        *ngIf="listProduct"
        class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 m-4"
      >
...
export class HomeComponent {
  ...
  applyFilterCategory(event: any) {
    this.listProduct = event.target.value
      ? this.allProducts.filter(
          (item) => item.category === event.target.value.toLowerCase()
        )
      : this.allProducts;
  }
  ...
  fetchAllProducts() {
    this.productService
      .getAllProduct()
      .subscribe((res) => (this.allProducts = this.listProduct = res));
  }
